...
const ListingForm = ({
  ...
  const [errors, setErrors] =
    useState<AxiosError<{ validationErrors: ValidationError[] }>>();
  ...
  const onSubmit = async (values: Partial<Listing>): Promise<void> => {
    ...
    const remainingProps: Omit<
      Listing,
      "clientId" | "estateId" | "contractId"
    > = formKeys.reduce((acc, current) => {
      if (
        !(
          current === "clientId" ||
          current === "estateId" ||
    ...
    try {
      const res =
        location.pathname === `/newlisting`
          ...
          : await http.put<Listing>(
              `editlisting/${userId}/${clientId}/${estateId}/${contractId}`,
              remainingProps
            );
      console.log(res);
      setListing(res.data);
              ...
              <Field name="district" component="input">
                {(props) => (
                  <>
                    ...
                    <ErrorMessage
                      fieldErrorMsg={selectValidationErrMsg(errors, "district")}
                    />
                  </>
                )}
              </Field>
